      ...
      var render = function render(res) {
        ...
        for (var j = 0, len2 = res.list.length; j < len2; j++) {
          ...
          for (var i = 0, len = data.link.length; i < len; i++) {
            var minSrc = 'http://oybu6iacu.bkt.clouddn.com/galleryMin/' + data.link[i];
            var src = 'http://oybu6iacu.bkt.clouddn.com/gallery/' + data.link[i];
            var type = data.type[i];
            var target = src + (type === 'video' ? '.mp4' : '.jpg');
            src += '';
  
            liTmpl += '<figure class="thumb" itemprop="associatedMedia" itemscope="" itemtype="http://schema.org/ImageObject">\
                  <a href="' + src + '" itemprop="contentUrl" data-size="1920x1280" data-curr-size="1920x1280" data-type="' + type + '" data-target="' + src + '">\
